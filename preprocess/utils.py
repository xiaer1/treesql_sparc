...
def generate_column_names_embedder_input(column_names, column_set, table_idxes, table_names):
    ...
    for i, (table_id, column_name) in enumerate(zip(table_idxes, column_names)):
        if table_id >= 0:
            ...
            # TODO *
            column_name_embedder_input = 'count number many'
            column_name_embedder_input_idx = (-1, 0)
    ...
    for i, table_name in enumerate(table_names):
        column_name_embedder_input = table_name + ' . count number many'
        column_names_embedder_input.append(column_name_embedder_input)
        column_name_embedder_input_idx = (i, 0)
        column_names_embedder_input_idxes.append(column_name_embedder_input_idx)
...
def load_dataSets(args):
    ...
    for i in range(len(table_datas)):
        ...
        table = table_datas[i]
        col_name_list = [col[1] if col[1] != '*' else 'count number many' for col in table['column_names']]

        col_names = []
        for y in col_name_list:
            x = [wordnet_lemmatizer.lemmatize(x.lower()) for x in y.split(' ')]
            col_names.append(" ".join(x))

        table_name_list = table['table_names']
        table_names = []
        for y in table_name_list:
            x = [wordnet_lemmatizer.lemmatize(x.lower()) for x in y.split(' ')]
            table_names.append(" ".join(x))

        table['table_names_iter'] = table_names
        ...
        # TODO 未去掉重复
        table['schema_content'] = col_names
        col_tables = [col[0] for col in table['column_names']]
        table['col_table'] = col_tables

        column_names_embedder_input, column_names_embedder_input_idxes = generate_column_names_embedder_input(
            col_names, tmp_col, col_tables, table_names)
    ...
    for d in datas:
        ...
        d['table_names'] = tables[d['database_id']]['table_names_iter']
