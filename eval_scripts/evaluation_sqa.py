...
record_correct = logger.Logger(os.path.join(log_dir, os.path.basename(__file__).split('.')[0] + '_correct.log'), 'w')
...
def evaluate(gold, predict, db_dir, etype, kmaps):
    ...
    # TODO plist = [[("select product_type_code from products group by product_type_code order by count ( * ) desc limit value", "orchestra")]]
    # TODO glist = [[("SELECT product_type_code FROM Products GROUP BY product_type_code ORDER BY count(*) DESC LIMIT 1", "customers_and_orders")]]
    evaluator = Evaluator()
    ...
    inter_count = len(plist)
    utter_count = 0
    for inter_idx, (p, g) in enumerate(zip(plist, glist)):

        # print('Interance {} [{} %]'.format(inter_idx, round(inter_idx / inter_count,3) * 100))
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            # print(g_str)
            # db = 'world_1'
            # p_str = 'SELECT sum ( T1.Population ) , avg ( T1.SurfaceArea ) FROM country AS T1 WHERE T1.Continent = "North America" and T1.SurfaceArea > 3000'
            # g_str = 'SELECT T1.owner_id ,  T1.last_name FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id  =  T2.owner_id JOIN Treatments AS T3 ON T2.dog_id  =  T3.dog_id GROUP BY T1.owner_id ORDER BY count(*) DESC LIMIT 1'
            # g_str = 'SELECT sum(Population) ,  avg(SurfaceArea) FROM country WHERE Continent  =  "North America" AND SurfaceArea  >  3000'
            ...
            if etype in ["all", "exec"]:
                ...
                if exec_score:
                    ...
                    pred = "CORRECT : {} pred: {}".format(hardness, p_str)
                    gold = "CORRECT : {} gold: {}".format(hardness, g_str)
                    record_correct.put(pred)
                    record_correct.put(gold)
                    record_correct.put('\n')
                else:
                    turn_scores['exec'].append(0)
                    pred = "{} pred: {}".format(hardness, p_str)
                    gold = "{} gold: {}".format(hardness, g_str)
                    # print(pred)
                    # print(gold)
                    # print()
                    record.put(pred)
    ...
    question_match,interaction_match = scores['all']['exec'] , scores['joint_all']['exec']
...
def eval_exec_match(db, p_str, g_str, pred, gold):
    ...
    """
    # db = 'sparc/database/student_transcripts_tracking/student_transcripts_tracking.sqlite'
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
    # print(db)
    # p_str = 'SELECT T1.transcript_date , T2.transcript_id FROM ' \
    #         'Transcript_Contents AS T2 JOIN Transcripts AS T1 ON T2.transcript_id  =  T1.transcript_id GROUP BY T2.transcript_id HAVING count ( * ) >= 2'
    try:
        cursor.execute(p_str)
        p_res = cursor.fetchall()

    except TimeoutException as e:
        print('Pred Execute time out!!!')
        return False
    except Exception as e:
        print('Error Pred query', p_str)
        return False
    # print(p_res)
    # g_str = 'SELECT T2.transcript_date ,  T1.transcript_id FROM Transcript_Contents AS T1 ' \
    #         'JOIN Transcripts AS T2 ON T1.transcript_id  =  T2.transcript_id GROUP BY T1.transcript_id HAVING count(*)  >=  2'
    try:
        cursor.execute(g_str)
        q_res = cursor.fetchall()

    except TimeoutException as e:
        print('Gold Execute time out!!!')
        return False
    except Exception as e:
        print('Error Gold query', g_str)
        return False
    # print(q_res)
