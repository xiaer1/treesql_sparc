...
def partial_match(query, table_name):
    query = [wordnet_lemmatizer.lemmatize(x) for x in query]
    table_name = [wordnet_lemmatizer.lemmatize(x) for x in table_name]
...
def is_partial_match(query, table_names):
    query = wordnet_lemmatizer.lemmatize(query)
    table_names = [[wordnet_lemmatizer.lemmatize(x) for x in names.split(' ') ] for names in table_names]
    same_count = 0
    result = None
    ...
    for d in datas:
        if 'C(0)' in d['model_result']:
            ...
            for q_ind, q in enumerate(d['question_arg']):
                q = [wordnet_lemmatizer.lemmatize(x) for x in q]
