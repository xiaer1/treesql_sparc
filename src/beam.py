    ...
    def check_sel_valid(self, actions):
        ...
        for ac in actions:
            if type(ac) == semQL.Sel:
                find_sel = True
            elif find_sel and type(ac) in [semQL.N, semQL.TC, semQL.A]:
                if type(ac) not in [semQL.N]:
                    sel_actions.append(ac)
            elif find_sel and type(ac) not in [semQL.N, semQL.TC, semQL.A]:
                break
...
if __name__ == '__main__':
    ...
    # print(semQL.Root1(3).get_next_action())
    test.actions.append(semQL.Root1(3))
    test.actions.append(semQL.Root(4))
    test.actions.append(semQL.Sel(0))
