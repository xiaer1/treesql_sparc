...
from eval_scripts.evaluation_sqa import build_foreign_key_map_from_json, evaluate
...
def get_candidate_tables(format_sql, schema):
  ...
  for ii, token in enumerate(tokens):
    if '.' in token:
      ...
      if table_name!='t1' or table_name!='t2' or table_name!='t3' or table_name!='t4' or table_name!='t5':
  ...
  candidate_tables_id = [table_names_original.index(table_name) for table_name in candidate_tables if table_name in table_names_original]
...
def postprocess_single(format_sql_2, schema, start_alias_id=0):
  ...
  #TODO [4, 7, 6] ['Breeds', 'Charges', 'Sizes', 'Treatment_Types', 'Owners', 'Dogs', 'Professionals', 'Treatments']
  ...
  if len(candidate_tables_id) == 0:
    ...
    # easy case
    #TODO 只有一个表，直接在select 后面 添加from，再把表名{}. 替换成 ’‘；
    ...
    #TODO table_alias_dict :  {4: 4, 5: 3, 6: 1, 7: 2} 对应表格：i 以及Ti位置
    # ret ： 返回的 from .. join .. on
    # from Professionals as T1 join Treatments as T2 on T1.professional_id = T2.professional_id join Dogs as T3 on T2.dog_id = T3.dog_id join Owners as T4 on T3.owner_id = T4.owner_id
    table_alias_dict, ret = gen_from(candidate_tables_id, schema)

    from_clause = ret

    #TODO 假如有嵌套 select ，那么第二个select后 的Ti 需要在第一个select后计数
    for i in range(len(table_alias_dict)):
      from_clause = from_clause.replace('T{}'.format(i+1), 'T{}'.format(i+1+start_alias_id))
    #TODO from Professionals as T1 join Treatments as T2 on T1.professional_id = T2.professional_id join Dogs as T3 on T2.dog_id = T3.dog_id join Owners as T4 on T3.owner_id = T4.owner_id
    table_name_to_alias = {}
    ...
    #TODO {'Professionals': 'T1', 'Treatments': 'T2', 'Owners': 'T4', 'Dogs': 'T3'}
    # print(table_name_to_alias)
...
def postprocess_nested(format_sql_2, schema):
  ...
  if len(candidate_tables_id) == 1:
    ...
    # TODO 把format_spl_2 添加 from形式
    #  ['select count ( Dogs.age )', 'from Dogs', 'where Dogs.age < =', '( select avg ( Dogs.age )', 'from Dogs', 'where Dogs.age = 1 )']

    final_sql = ' '.join(format_sql_3).replace('{}.'.format(table_name), '')
    #TODO select count ( age ) from Dogs where age < = ( select avg ( age ) from Dogs where age = 1 )
    ...
    def postprocess_subquery(sub_query_one, schema, start_alias_id_1):
      ...
      for sub_sql in sub_query_one.split('\n'):
        #TODO 这是干嘛的？  首先要是select字句，而且不是第一个select
        if 'select' in sub_sql:
          if len(sub_query) > 0:
...
def postprocess(predictions, database_schema, remove_from=False):
  ...
  for pred in predictions:
    #TODO {'flat_prediction': ['select', 'documents.*'], 'gold_query': ['select', 'documents.*'],
    #  'prediction': ['select', 'documents.*', '_EOS'], 'flat_gold_queries': [['select', 'documents.*']]
    #dict_keys(
    #      ['input_seq', 'probability', 'index_in_interaction', 'interaction_id', 'flat_gold_queries', 'database_id',
    #       'flat_prediction', 'prediction', 'gold_query', 'gold_tables', 'identifier'])

    db_id = pred['database_id']
    schema = database_schema[db_id]
    # dict_keys(['table_names_original', 'table_names', 'primary_keys', 'foreign_keys',
    # 'column_names_original', 'column_names', 'db_id', 'column_types'])
    # print(schema.keys())
...
def write_and_evaluate(postprocess_sqls, db_path, table_schema_path, gold_path, dataset):
  ...
  # TODO ['flight_2', 'pets_1', 'world_1', 'tvshow', 'poker_player', 'battle_death', 'car_1', 'wta_1', 'orchestra',
  #  'cre_Doc_Template_Mgt', 'concert_singer', 'singer', 'employee_hire_evaluation', 'museum_visit', 'network_1', 'course_teach', 'real_estate_properties', 'voter_1', 'student_transcripts_tracking', 'dog_kennels']
  # 20 == 20
  #print(len(postprocess_sqls.keys()),len(db_list))
  ...
  if dataset == 'spider':
    ...
    # command = 'python3 src/eval_scripts/evaluation.py --db {} --table {} --etype match --gold {} --pred {}'.format(db_path,
    #                                                                                               table_schema_path,
    #                                                                                               gold_path,
    ...
    with open(output_file, "w") as f:
      ...
      for db in db_list:
        ...
        if db not in postprocess_sqls.keys():
            continue
        for postprocess_sql, interaction_id, turn_id in postprocess_sqls[db]:
    ...
    etype = 'exec'
    question, interaction = evaluate(gold_path, os.path.abspath(output_file), db_path, etype, kmaps)
...
if __name__ == '__main__':
  ...
  # python3 postprocess_eval.py --dataset=sparc --split=dev --pred_file predictions_asc.json --remove_from
  db_path = 'sparc/database/'

  table_schema_path = 'sparc/tables.json'
  if args.split == 'dev':
      gold_path = 'sparc/dev_gold.txt'
  ...
  #print('Generate predicted file : predicted_sql.txt')
