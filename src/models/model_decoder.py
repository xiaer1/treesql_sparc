...
class EditTreeNet(BasicModel):
    def __init__(self,args,grammar):
        ...
        #         self.schema_encoder_2 = Encoder(schema_encoder_num_layer, self.schema_attention_key_size,
        #                                         self.schema_attention_key_size)
        #         # TODO : self.utterance_attention_key_size = 650
        #         self.utterance_encoder_2 = Encoder(args.encoder_num_layers, self.utterance_attention_key_size,
        #                                            self.utterance_attention_key_size)
        ...
        if self.args.use_copy_switch:
            if self.args.use_query_attention:
                #TODO 300 + 650  + 300 ,1
                self.state2copyswitch_transform_weights_sketch = torch_utils.add_params((sketch_att_vec_linear_input, 1), "weights-state-transform_sketch")
    ...
    def forward(self,idx,epoch,interaction,optimizer,bert_optimizer):
        ...
        for utterance_index, utterance in enumerate(utterances):
            ...
            if self.args.use_previous_query and len(previous_lf) > 0:
                previous_lf_queries, previous_lf_query_states = self.get_previous_queries(previous_lf_queries,previous_lf_query_states, previous_lf,interaction)
                #TODO [[Root1(3), Root(5), Sel(0), N(1), A(none), TC(15), A(none), TC(18)]]
                # 8 torch.Size([300])
                # print(previous_lf_queries)
                # print(len(previous_lf_query_states[0]),previous_lf_query_states[0][0].size())
            ...
            lf_prob_var,lf_gold_token_cnt = self.train_action_one_turn(utterance,utterance_states,final_utterance_state,interaction,schema_states,previous_lf_queries,previous_lf_query_states)
    ...
    def get_utterance_schema_attention(self,schema_states,utterance_states):
        ...
        if self.args.use_encoder_attention:
            ...
            # torch.Size([350, 28])
            schema_attention = self.utterance2schema_attention_module(torch.stack(schema_states, dim=0),
                                                                      utterance_states).vector
            # torch.Size([300, 6])
            utterance_attention = self.schema2utterance_attention_module(torch.stack(utterance_states, dim=0),
                                                                         schema_states).vector
            ...
            if self.args.use_schema_encoder_2:
                final_schema_state, schema_states = self.schema_encoder_2(schema_states, lambda x: x,
                                                                          dropout_amount=self.dropout_ratio)
                final_utterance_state, utterance_states = self.utterance_encoder_2(utterance_states,
                                                                                   lambda x: x,
                                                                                   dropout_amount=self.dropout_ratio)
        if isinstance(schema_states, list):
            schema_states = torch.stack(schema_states, dim=0).unsqueeze(0)
    ...
    def get_previous_queries(self, previous_queries, previous_query_states, previous_query,interaction):
        ...
        # TODO Root1(3) <class 'src.rule.semQL.Root1'>
        # for action in previous_query:
        #     print(action,type(action),type(action) == define_rule.Root1)

        query_token_embedder = lambda query_token : self.get_query_token_embedding(query_token,interaction)
    ...
    def get_query_token_embedding(self, output_token, interaction):
        '''
        :param output_token:
        :param interaction:
        :return:
    ...
    def predict_with_gold_queries(self,interaction,beam_size=1,use_gold_query=True):
        ...
        for utterance_index, utterance in enumerate(utterances):
            ...
            #torch.Size([6, 300])
            # torch.Size([29, 300])
            # ['*', 'airlines . airline id', 'airlines . airline name', 'airlines . abbreviation', 'airlines . country', 'airports . city', 'airports . airport code', 'airports . airport name', 'airports . country', 'airports . country abbrev', 'flights . airline', 'flights . flight number', 'flights . source airport', 'flights . destination airport', 'airlines . *', 'airports . *', 'flights . *', 'airline id', 'airline name', 'abbreviation', 'country', 'city', 'airport code', 'airport name', 'country abbrev', 'airline', 'flight number', 'source airport', 'destination airport']
            # 29
            # print(torch.stack(utterance_states).size())
            # print(torch.stack(schema_states).size())
            # print(interaction.column_names_embedder_input)
            # print(len(interaction.column_names_embedder_input))
            input_hidden_states.extend(utterance_states)
            input_sequences.append(input_sequence)
            num_utterances_to_keep = min(self.args.maximum_utterances, len(input_sequences))
            if self.args.discourse_level_lstm:
        ...
        for utterance_index, utterance in enumerate(utterances):
            ...
            if completed_sketch_beams == []:
                ...
                completed_lf_beams = self.predict_action_one_turn(beam_size, interaction, padding_sketch,
                                                                  ...
                                                                  schema_states, previous_lf_queries,
                                                                  previous_lf_query_states)
                if completed_lf_beams == []:
                    lf_actions = []
                else:
    ...
    def predict_sketch_one_turn(self,beam_size,utterance_states,final_utterance_state,previous_sketch_queries,previous_sketch_query_states):
        ...
        while (len(completed_beams)) < beam_size and  t < self.args.decode_max_time_step:
            ...
            if self.args.use_previous_query and len(previous_sketch_queries) > 0:
                ...
                previous_sketch_query = previous_sketch_queries[0]
                previous_sketch_query_state = previous_sketch_query_states[0]

                previous_sketch_query_state = torch.stack(previous_sketch_query_state, dim=1)
                # TODO torch.Size([300, N])

                query_scores, query_tokens = self.score_query_tokens(previous_sketch_query, previous_sketch_query_state,
                                                                     self._get_query_token_scorer(intermediate_state))
                query_scores = query_scores.squeeze()
                query_scores = torch.log_softmax(query_scores, dim=0)
                    ...
                    for possible_production in possible_productions:
                        ...
                        if self.args.use_previous_query and len(previous_sketch_queries) > 0:
                            ...
                            for i,item in enumerate(previous_sketch_query):
                                if item.production == possible_production:
    ...
    def predict_action_one_turn(self,beam_size,interaction,padding_sketch,utterance_states,final_utterance_state,schema_states,previous_lf_queries,previous_lf_query_states):
        ...
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            ...
            if att_t.dim() == 1:
                att_t = att_t.unsqueeze(0)
            apply_rule_log_prob = torch.log_softmax(self.production_readout(att_t), dim=-1)
            schema_weight = torch.bmm(schema_states, self.schema_att_vec_linear(state_and_attn.unsqueeze(0)).unsqueeze(2)).squeeze(2)
            ...
            if self.args.use_previous_query and len(previous_lf_queries) > 0:
                if self.args.use_copy_switch:
                    copy_switch = self._get_copy_switch_lf(state_and_attn)

                assert len(previous_lf_queries) == 1, 'debug mode'
                previous_lf_query = previous_lf_queries[0]
                previous_lf_query_state = previous_lf_query_states[0]
                previous_lf_query_state = torch.stack(previous_lf_query_state, dim=1)
                # TODO torch.Size([300, N])

                query_scores, query_tokens = self.score_query_tokens(previous_lf_query, previous_lf_query_state,
                                                                     self._get_query_token_scorer(intermediate_state))
                query_scores = query_scores.squeeze()
                query_scores = torch.log_softmax(query_scores, dim=0)
            ...
            if type(padding_sketch[t]) == define_rule.A:
                ...
                for tc_id, _ in enumerate(interaction.column_names_embedder_input):
                    ...
                    if self.args.use_previous_query and len(previous_lf_queries) > 0:
                        ...
                        for i, item in enumerate(previous_lf_query):
                            if type(item) == define_rule.TC and item.id_c == tc_id:
                                query_scores_list.append(query_scores[i])
    ...
    def train_action_one_turn(self,utterance,utterance_states,final_utterance_state,interaction,schema_states,previous_lf_queries,previous_lf_query_states):
        ...
        for t in range(utterance.action_num):
            ...
            att_t = torch.tanh(self.lf_att_vec_linear(state_and_attn))
            att_t = self.dropout_layer(att_t)
            apply_rule_prob = torch.softmax(self.production_readout(att_t), dim=-1).squeeze()

            schema_weight = torch.bmm(schema_states, self.schema_att_vec_linear(state_and_attn.unsqueeze(0)).unsqueeze(2)).squeeze(2)
            schema_weight = F.softmax(schema_weight, dim=-1).squeeze()
            ...
            if self.args.use_previous_query and len(previous_lf_queries) > 0:
                ...
                previous_lf_query_state = previous_lf_query_states[0]
                previous_lf_query_state = torch.stack(previous_lf_query_state, dim=1)
                # TODO torch.Size([300, 4])

                query_scores, query_tokens = self.score_query_tokens(previous_lf_query, previous_lf_query_state,
                                                                     self._get_query_token_scorer(intermediate_state))
                query_scores = query_scores.squeeze()
                query_scores = F.softmax(query_scores, dim=0)
            ...
            if len(previous_lf_queries) > 0:
                ...
                query_scores_list = []
                for i,item in enumerate(previous_lf_query):
                    if isinstance(action_t,define_rule.A) or isinstance(action_t,define_rule.TC):
                        if str(item) == str(action_t):
            ...
            if isinstance(action_t, define_rule.TC):
                ...
                tgt_action_probs.append(act_prob_t_i + noise)
                gold_token_count +=1
    ...
    def train_sketch_one_turn(self,utterance,utterance_states,final_utterance_state,previous_sketch_queries,previous_sketch_query_states):
        ...
        for t in range(utterance.sketch_num):
            ...
            if self.args.use_previous_query and len(previous_sketch_queries) > 0:
                ...
                # print(previous_sketch_query_state.size())
                # print(previous_sketch_query)
                query_scores, query_tokens = self.score_query_tokens(previous_sketch_query,previous_sketch_query_state,self._get_query_token_scorer(intermediate_state))
                query_scores = query_scores.squeeze()
                query_scores = F.softmax(query_scores, dim=0)
    ...
    def _get_copy_switch_sketch(self, state):
        copy_switch = torch.sigmoid(torch_utils.linear_layer(state, self.state2copyswitch_transform_weights_sketch))

        return copy_switch.squeeze()

    def _get_copy_switch_lf(self, state):
