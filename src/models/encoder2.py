...
from .torch_utils import create_multilayer_lstm_params, encode_sequence2
...
class Encoder2(torch.nn.Module):
    ...
    def forward(self, sequence, embedder, dropout_amount=0.):
        ...
        """
        forward_state, forward_outputs = encode_sequence2(
            sequence,
            self.forward_lstms,
            embedder,
            dropout_amount=dropout_amount)

        backward_state, backward_outputs = encode_sequence2(
            sequence[::-1],
            self.backward_lstms,
            embedder,
            dropout_amount=dropout_amount,inv=True)
