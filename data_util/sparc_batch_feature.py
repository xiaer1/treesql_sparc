...
from src.rule.semQL import Sup, Sel, Order, Root, Filter, A, N, Root1,TC
import copy
import numpy as np
...
class UtteranceItem():
    def __init__(self,utterance : dict,last_utter : dict,_type = 'train'):
        ...
        self.one_utterance = utterance['utterance_toks']
        self.one_utterance_linking = utterance['utterance_arg_linking']
        self.schema_feature = utterance['schema_feature']
        if last_utter == None:
            self.union_utterance = utterance['utterance_arg_linking']
            # self.question_feature = utterance['question_feature']
        else:
            if tab_cnt!=0 and col_cnt!=0:
                 self.union_utterance = utterance['utterance_arg_linking']
                 # self.question_feature = utterance['question_feature']
            else:
                # self.question_feature = np.concatenate([last_utter['question_feature'],
                #                                         utterance['question_feature']], axis=0)
                ...
                # assert self.question_feature.shape[0] == len(self.union_utterance)
        ...
        if _type == 'train':
            ...
            if self.target_actions:
                for action in self.target_actions:
                    if isinstance(action,define_rule.TC) or isinstance(action,define_rule.A) :
                            continue
